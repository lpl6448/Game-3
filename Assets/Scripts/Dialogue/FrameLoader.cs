...
public class FrameLoader : MonoBehaviour
{
    ...
    /// <returns></returns>
    public List<DialogueFrame> LoadDialogue(DLists list, TextAsset targetFile)
    {
        //Fill selected list with values pulled from JSON files and return the handled info as a list of Dialogue Frames
        switch(list)
        {
            case DLists.Intro:
                introList = JsonUtility.FromJson<FrameList>(targetFile.text);
            ...
            case DLists.Molly:
                ...
                {
                    mollyDL.Add(new DialogueFrame(mf.ID, mf.speaker, mf.emotion, mf.line, mf.lineType, mf.nextFrame1, mf.nextFrame2, mf.response1, mf.response2));
                }
                return mollyDL;
            case DLists.Marcone:
                marconeList = JsonUtility.FromJson<FrameList>(targetFile.text);
                foreach (MockFrame mf in marconeList.list)
                {
                    marconeDL.Add(new DialogueFrame(mf.ID, mf.speaker, mf.emotion, mf.line, mf.lineType, mf.nextFrame1, mf.nextFrame2, mf.response1, mf.response2));
                }
                return marconeDL;
            case DLists.LT:
                ltList = JsonUtility.FromJson<FrameList>(targetFile.text);
                foreach (MockFrame mf in ltList.list)
                {
                    lcDL.Add(new DialogueFrame(mf.ID, mf.speaker, mf.emotion, mf.line, mf.lineType, mf.nextFrame1, mf.nextFrame2, mf.response1, mf.response2));
                }
                return lcDL;
